...
public abstract class AbstractPathMatchEventHandler<T extends DisruptorEvent> extends AbstractAdviceEventHandler<T> implements PathProcessor<T> {
     ...
     * 需要过滤的路径
     */
    private List<String> appliedPaths = new ArrayList<String>();
    ...
    private String getPathWithinEvent(T event) {
        return event.getRouteExpression();
    }

    private boolean pathsMatch(String path, T event) {
    ...
    private boolean onPreHandle(T event) throws Exception {
        return true;
    }

    private boolean isEnabled(T event, String path) throws Exception {
